...
const recordPromiseFactory = (IDs: string[]): Promise<PublicRecordType[]> => new Promise(
    (res, _) => res(
        IDs.map(id => fakeDocFactory(id))
    )
)
...
describe("Records", () => {
    ...
    test("They should accept both and generate request urls", () => {
        const result = new PublicRecord()
            .addRecord(fakeDocFactory("FAKE_ID"))
            .addRecord("FAKE_ID")
    ...
    test("They should accept both and generate request urls from adding multiple", () => {
        const result = new PublicRecord()
            .addMultiple([fakeDocFactory("FAKE_ID"), "FAKE_ID"])
        expect(result.requestUrls().length).toBe(2)
    })
    ...
    test("Can add a Promise to doc search", () => {
        ...
        return new PublicRecord()
            .addRecordPromise(
                recordPromiseFactory(["FAKE_ID_1", "FAKE_ID_2", "FAKE_ID_3"]),
            )
